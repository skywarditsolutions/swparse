...
from swparse.lib import dependencies
...
async def test_provide_user_dependency() -> None:
    user = User()
    request = RequestFactory(swparse=Litestar(route_handlers=[])).get("/", user=user)
...
async def test_provided_filters(swparse: Litestar, client: AsyncTestClient) -> None:
    called = False
    path = f"/{uuid4()}"
    ...
    swparse.register(filtered_collection_route)
...
async def test_filters_dependency(swparse: "Litestar", client: "AsyncTestClient") -> None:
    called = False
    path = f"/{uuid4()}"
    ids = [uuid4() for _ in range(2)]
...
async def test_filters_dependency_no_ids(swparse: "Litestar", client: "AsyncTestClient") -> None:
    called = False
    path = f"/{uuid4()}"
