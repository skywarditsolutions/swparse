...
def _patch_db(
    ...
    if isinstance(alchemy._config, list):
        monkeypatch.setitem(swparse.state, alchemy._config[0].engine_app_state_key, engine)
        monkeypatch.setitem(
            swparse.state,
        ...
        monkeypatch.setitem(swparse.state, alchemy._config.engine_app_state_key, engine)
        monkeypatch.setitem(
            swparse.state,
...
@pytest.fixture(autouse=True)
def _patch_redis(swparse: "Litestar", redis: Redis, monkeypatch: pytest.MonkeyPatch) -> None:
    cache_config = swparse.response_cache_config
    assert cache_config is not None
    saq_plugin = get_saq_plugin(swparse)
    app_plugin = swparse.plugins.get(ApplicationConfigurator)
    monkeypatch.setattr(app_plugin, "redis", redis)
    monkeypatch.setattr(swparse.stores.get(cache_config.store), "_redis", redis)
...
async def fx_client(swparse: Litestar) -> AsyncIterator[AsyncClient]:
    """Async client that calls requests on the swparse.
