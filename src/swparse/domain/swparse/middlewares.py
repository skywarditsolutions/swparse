...
class ApiKeyAuthMiddleware(AbstractMiddleware):
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        logger.error(Request(scope).headers)
        api_key = Request(scope).headers.get(api_key_header)
        if not api_key:
        ...
        api_key_service = await anext(provide_api_key_service(alchemy.provide_session(connection.app.state, connection.scope)))
        is_authorized = await api_key_service.authenticate(api_key)
